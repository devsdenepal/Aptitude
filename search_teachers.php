...
$query=$_GET['search'];
$sql = "SELECT Slno, FirstName, Subject,LastName,  Age FROM teachers WHERE LastName like'%$query%' or FirstName like'%$query%'";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
  // output data of each row
  while($row = $result->fetch_assoc()) {
    echo "<tr><td> " . $row["Slno"]. " </td><td>" . $row["FirstName"]. "</td><td>" . $row["LastName"]. "</td><td>" . $row["Age"].  "</td><td>". $row['Subject'] . "</td><td>". "<button style='color:white;background-color:blue;'><a href='update.php?Slno=". $row['Slno']." '><i class='fa-solid fa-edit'></i> edit</a></button>". "<button style='color:white;background-color:red;'><a href='delete.php?Slno=". $row['Slno']." '><i class='fa-solid fa-trash'></i> delete</a></button><br></td></tr>";
  }
